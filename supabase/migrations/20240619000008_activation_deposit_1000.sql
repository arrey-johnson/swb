-- Activation deposit and reserve floor: 1,000 FCFA (was 5,000)

ALTER TABLE public.savings_accounts DROP CONSTRAINT IF EXISTS reserve_balance_fixed;

UPDATE public.savings_accounts SET reserve_balance = 1000.00 WHERE reserve_balance = 5000.00;

ALTER TABLE public.savings_accounts ALTER COLUMN reserve_balance SET DEFAULT 1000.00;
ALTER TABLE public.savings_accounts ADD CONSTRAINT reserve_balance_fixed CHECK (reserve_balance = 1000);
...
BEGIN
  ...
  VALUES (NEW.id, 0.00, 1000.00, 'pending');
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'reserve_balance', COALESCE(v_account.reserve_balance, 1000),
    'account_status', COALESCE(v_account.status::TEXT, 'pending'),
    'is_active', v_is_active,
    'activation_required', COALESCE(v_account.status = 'pending', true),
    'activation_remaining', GREATEST(0, COALESCE(v_account.reserve_balance, 1000) - COALESCE(v_account.balance, 0)),
    'max_withdrawable', CASE WHEN v_is_active THEN GREATEST(0, COALESCE(v_account.balance, 0) - COALESCE(v_account.reserve_balance, 1000)) ELSE 0 END,
